...
- Latest implementation/coverage commit before this handoff document: `247e777 feat(runtime): support stdio ungetc pushback`
...
- `getchar` and `fgetc` currently consume interpreter-local `ungetc` pushback and otherwise return `EOF`; they do not read from a host input stream. `feof` still returns zero in the current status skeleton.
