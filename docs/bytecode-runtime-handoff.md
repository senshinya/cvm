...
- Latest implementation/coverage commit before this handoff document: `8993e78 feat(runtime): execute empty stdio character input`
...
- `getchar` and `fgetc` currently model empty input only: they return `EOF` and do not read from a host input stream. `feof` still returns zero in the current status skeleton.
