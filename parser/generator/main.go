...
func checkProductions(prods *Productions) {
	...
	for _, prod := range prods.Productions {
		for _, part := range prod.Right {
			if entity.IsTerminalSymbol(part) {
				continue
			}
...
func closure(kernel []LRItem, productions *Productions) []LRItem {
	...
	for stack.Size() != 0 {
		...
		if entity.IsTerminalSymbol(left) {
			continue
		}
...
const (
{{ range $_, $v := .NonTerminals }}	{{ $v }} entity.TokenType = "{{ $v }}"
{{ end }}
)

var Productions = []entity.Production{
{{ range $_, $v := .Productions }}	{ Left: {{if isTerminal $v.Left}}entity.{{end}}{{ $v.Left }}, Index: {{ $v.Index }}, Right: []entity.TokenType{ {{ range $i, $r := $v.Right }}{{if $i}},{{end}}{{if isTerminal $r}}entity.{{end}}{{ $r }}{{ end }} } },
{{ end }}
}
...
var LalrGoto = map[int]map[entity.TokenType]int{
...
`
	...
	err = template.Must(template.New("test").Funcs(template.FuncMap{"isTerminal": entity.IsTerminalSymbol}).Parse(tmpl)).Execute(&buf, data)
	if err != nil {
		panic(err)
	}
