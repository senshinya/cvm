...
var lalr_table = map[int]map[string]DFAOperator{
{{ range $index, $ops := .Table }}	{{ $index }}: { {{ $first := true }}{{ range $k, $op := $ops }}{{if not $first}}, {{else}}{{$first = false}}{{end}}"{{ $k }}": { {{ if eq $op.OperatorType 1 }}OperatorType: SHIFT, StateIndex: {{ $op.StateIndex }}{{ else if eq $op.OperatorType 2 }}OperatorType: REDUCE, ReduceIndex: {{ $op.ReduceIndex }}{{ else }}OperatorType: ACC{{ end }} }{{end}}},
{{ end }}
}
`
	data := struct {
	...
	err = template.Must(template.New("test").Parse(tmpl)).Execute(&buf, data)
	if err != nil {
		panic(err)
	}
