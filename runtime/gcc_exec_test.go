...
func TestGCCFopenConfiguredFileWriteModeExecutesThroughRuntime(t *testing.T) {
	source := `/* { dg-do run } */
#include <stdio.h>
...
int main(void)
{
  ...
  if (fread(buf, 1, 3, f) != 3)
    return 6;
  return buf[0] == 'A' && buf[1] == 'B' && buf[2] == 'C' ? 0 : 7;
}
`
	reg := DefaultExternRegistry(nil, nil)
	st := runGCCExecFixtureWithLoadOptions(t, "stdio-fopen-configured-file-write-runtime.c", source, gccExecStepLimit, LoadOptions{Externs: reg})
	if st.Code != 0 {
		t.Fatalf("exit code = %d, want 0", st.Code)
	}
}
...
func TestGCCInlineVolatileCallsExecuteThroughRuntime(t *testing.T) {
	st := runGCCAcceptFixtureWithStepLimit(t, "sema/testdata/gcc-c99-extra/accept/pr71969-1.c", 1000000)
	if st.Code != 0 {
		t.Fatalf("exit code = %d, want 0", st.Code)
	}
}
...
int main(void)
{
  ...
  if (fma(2.0, 3.0, 4.0) != 10.0 || fmaf(2.0f, 3.0f, 4.0f) != 10.0f || fmal(2.0L, 3.0L, -4.0L) != 2.0L)
    return 42;
  if (nextafter(1.0, 1.0) != 1.0 || nextafterf(1.0f, 1.0f) != 1.0f || nextafterl(1.0L, 1.0L) != 1.0L)
    return 43;
  if (nexttoward(1.0, 1.0L) != 1.0 || nexttowardf(1.0f, 1.0L) != 1.0f || nexttowardl(1.0L, 1.0L) != 1.0L)
    return 44;
...
int main(void)
{
  ...
  n = sprintf(buf, "%hhd %hd %d %ld %lld %jd %td", 255, 65534, -3, -4L, -5LL, (intmax_t)-6, (ptrdiff_t)-7);
  if (n != 20)
    return 3;
  ...
  n = sprintf(buf, "%8.2f|%-8.1e|%8.1E|%.3g|%.3G|%12.2a|%-12.2A", 1.5, 12.0, 12.0, 12345.0, 12345.0, 1.5, 1.5);
  if (n != 70)
    return 22;
...
func TestPlainSscanfSignedLengthsExecuteThroughRuntime(t *testing.T) {
	source := `/* { dg-do run } */
#include <stdio.h>
#include <stddef.h>
#include <stdint.h>
...
int main(void)
{
  ...
  long cz = 0;
  ptrdiff_t ct = 0;
  int n = sscanf("-1 -2 -3 -4 -5 -6 -7 -8", "%hhd %hd %d %ld %lld %jd %zd %td", &c8, &c16, &c32, &cl, &cll, &cj, &cz, &ct);
  if (n != 8)
    return 1;
  return c8 == -1 && c16 == -2 && c32 == -3 && cl == -4 && cll == -5 && cj == -6 && cz == -7 && ct == -8 ? 0 : 2;
}
`
	st := runGCCExecFixture(t, "plain-sscanf-signed-lengths-runtime.c", source)
	if st.Code != 0 {
		t.Fatalf("exit code = %d, want 0", st.Code)
	}
}
...
func TestPlainSscanfCharacterLengthsExecuteThroughRuntime(t *testing.T) {
	source := `/* { dg-do run } */
#include <stdio.h>
#include <wchar.h>
...
int main(void)
{
  char c = 0;
  wchar_t w[2] = { 0, 0 };
  int n = sscanf("ABC", "%c%2lc", &c, w);
  if (n != 2)
    return 1;
  return c == 'A' && w[0] == L'B' && w[1] == L'C' ? 0 : 2;
}
`
	st := runGCCExecFixture(t, "plain-sscanf-character-lengths-runtime.c", source)
	if st.Code != 0 {
		t.Fatalf("exit code = %d, want 0", st.Code)
	}
}
...
func TestPlainSscanfScansetLengthsExecuteThroughRuntime(t *testing.T) {
	source := `/* { dg-do run } */
#include <stdio.h>
#include <string.h>
#include <wchar.h>
...
int main(void)
{
  wchar_t wide[4] = { 0, 0, 0, 0 };
  char narrow[4] = { 0, 0, 0, 0 };
  int n = sscanf("abc123Z", "%3l[a-z]%3[^Z]", wide, narrow);
  if (n != 2)
    return 1;
  if (!(wide[0] == L'a' && wide[1] == L'b' && wide[2] == L'c' && wide[3] == 0))
    return 2;
  return strcmp(narrow, "123") == 0 ? 0 : 3;
}
`
	st := runGCCExecFixture(t, "plain-sscanf-scanset-lengths-runtime.c", source)
	if st.Code != 0 {
		t.Fatalf("exit code = %d, want 0", st.Code)
	}
}
...
func TestComplexInverseHyperbolicExecutesThroughRuntime(t *testing.T) {
	source := `/* { dg-do run } */
#include <complex.h>
...
int main(void)
{
  ...
  double complex od = __builtin_complex(1.0, 0.0);
  long double complex ol = __builtin_complex(1.0L, 0.0L);
  if (crealf(casinhf(zf)) != 0.0f || cimagf(casinhf(zf)) != 0.0f)
    return 1;
  if (creal(casinh(zd)) != 0.0 || cimag(casinh(zd)) != 0.0)
    return 2;
  if (creall(casinhl(zl)) != 0.0L || cimagl(casinhl(zl)) != 0.0L)
    return 3;
  if (crealf(cacoshf(of)) != 0.0f || cimagf(cacoshf(of)) != 0.0f)
    return 4;
  if (creal(cacosh(od)) != 0.0 || cimag(cacosh(od)) != 0.0)
    return 5;
  if (creall(cacoshl(ol)) != 0.0L || cimagl(cacoshl(ol)) != 0.0L)
    return 6;
  if (crealf(catanhf(zf)) != 0.0f || cimagf(catanhf(zf)) != 0.0f)
    return 7;
  if (creal(catanh(zd)) != 0.0 || cimag(catanh(zd)) != 0.0)
    return 8;
  return creall(catanhl(zl)) == 0.0L && cimagl(catanhl(zl)) == 0.0L ? 0 : 9;
}
`
	st := runGCCExecFixture(t, "complex-inverse-hyperbolic.c", source)
	if st.Code != 0 {
		t.Fatalf("exit code = %d, want 0", st.Code)
	}
}

func TestComplexExpLogSqrtExecutesThroughRuntime(t *testing.T) {
	source := `/* { dg-do run } */
#include <complex.h>
...
int main(void)
{
  ...
  double complex od = __builtin_complex(1.0, 0.0);
  long double complex ol = __builtin_complex(1.0L, 0.0L);
  if (crealf(cexpf(zf)) != 1.0f || cimagf(cexpf(zf)) != 0.0f)
    return 1;
  if (creal(cexp(zd)) != 1.0 || cimag(cexp(zd)) != 0.0)
    return 2;
  if (creall(cexpl(zl)) != 1.0L || cimagl(cexpl(zl)) != 0.0L)
    return 3;
  if (crealf(clogf(of)) != 0.0f || cimagf(clogf(of)) != 0.0f)
    return 4;
  if (creal(clog(od)) != 0.0 || cimag(clog(od)) != 0.0)
    return 5;
  if (creall(clogl(ol)) != 0.0L || cimagl(clogl(ol)) != 0.0L)
    return 6;
  if (crealf(csqrtf(zf)) != 0.0f || cimagf(csqrtf(zf)) != 0.0f)
    return 7;
  if (creal(csqrt(zd)) != 0.0 || cimag(csqrt(zd)) != 0.0)
    return 8;
  return creall(csqrtl(zl)) == 0.0L && cimagl(csqrtl(zl)) == 0.0L ? 0 : 9;
}
`
	st := runGCCExecFixture(t, "complex-exp-log-sqrt.c", source)
	if st.Code != 0 {
		t.Fatalf("exit code = %d, want 0", st.Code)
	}
}
