...
`, Options{})
	...
	for _, name := range []string{"abs", "labs", "llabs", "atoi", "atol", "atoll", "malloc", "calloc", "free", "strdup", "exit", "abort"} {
		if !hasIdentifier(res.Tokens, name) {
...
`, Options{})
	...
	for _, name := range []string{"size_t", "strcmp", "memcmp", "strncmp", "strlen", "strchr", "strstr", "memchr", "memcpy", "memmove", "mempcpy", "memset", "bzero", "strcpy", "stpcpy", "strcat", "strncpy", "stpncpy", "strncat"} {
		if !hasIdentifier(res.Tokens, name) {
